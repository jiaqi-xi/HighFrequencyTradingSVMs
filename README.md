...
This is an Algorithmic Trading Project.
...
3. Run `SVM Model Tuning.ipynb`.
   - Do gridsearch and store results in `./data/gridsearch_results.pkl`.
4. Run `Backtesting.ipynb`.
   - Manually input the best parameters and do backtesting.
   - Generate plots in `./plots/backtesting`.

There is something wrong within `BacktestingParallelized_1_26.ipynb` in its original version. We can use the `Backtesting.ipynb` one first.
